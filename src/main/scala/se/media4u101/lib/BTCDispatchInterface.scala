...
trait BTCDispatchInterface extends Loggable{
  ...
  protected def doFetchAccProfileData(poolAccountProfile:Box[String]):JValue = {
    ...
    val json = parse((response().right).getOrElse(this.handleResonsError("doFetchAccProfileData",response().left.get)))
...
//  }    
  ...
  protected def doFetchSlushPoolStat():JValue = {
    ...
    val response = Http(myRequest OK as.String).either
    val json = parse((response().right).getOrElse(this.handleResonsError("doFetchSlushPoolStat",response().left.get)))  
    val stop: Long = System.currentTimeMillis
  ...
  protected def doFetchWalletData(blockchainInfoAddress:Box[String]):JValue = {
    ...
    val response = Http(myRequest OK as.String).either
    val json:JValue = parse((response().right).getOrElse(this.handleResonsError("doFetchWalletData",response().left.get))) 
    val stop: Long = System.currentTimeMillis
  ...
  protected def doFetchMtgoxSEK():JValue = {
    ...
    val response = Http(myRequest OK as.String).either
    val json = parse((response().right).getOrElse(this.handleResonsError("doFetchWalletData",response().left.get)))  
    val stop: Long = System.currentTimeMillis
  ...
  private def handleResonsError(methodName:String,err:Throwable):String = {
    val errmes = err.getMessage();
    logger.error("BTCDispatchInterface::"+methodName+" "+errmes)
    errmes
  }  
}